...
boton.addEventListener('click', () => {
	...
	// Esta función retorna una promesa, por eso capturamos su resultado con el .then()
	
	// 2- La funcion debe ser llamada en el lugar correspondiente.
   consultaAsincrona("endpoint").then( response => renderizarElementos(response) )
})
...
function consultaAsincrona(texto) {
	return new Promise((resolve, reject) => {
		...
		setTimeout(function() {
			// chequeamos que el texto sea el correcto, esto simula lo que a futuro vamos a hacer con fetch()
			texto === "endpoint"
				// si está OK devolvemos el listado
				? resolve(listadoComentarios)
				// otra posibilidad es que la promesa sea rechazada
				: reject({ mensaje: "Consulta rechazada."})
		}, 2500)
	}) 
}
...
/* -------------------------------------------------------------------------- */
// En este caso la consigna será más abierta, se explicitarán los requerimientos pero hay varias maneras de llegar al resultado.
// Muchos éxitos!
...
function renderizarElementos(listado){
	...
	// 			<p>${post.body}</p>
	// 		</div>`
	// 	divPosts.innerHTML += postTemplate
	// })

	// ########## Renderizado usando .map() ##########
	divPosts.innerHTML = listado.map( post => `
		<div class="comentario">
			<h4>${post.email}</h4>
			<p>${post.body}</p>
		</div>`	).join("")
