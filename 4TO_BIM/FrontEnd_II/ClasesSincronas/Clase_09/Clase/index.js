...
form.addEventListener("submit", e => {
  ...
  if (!emailRegex.test(user.email)) { errores.push("Mail no válido") }
  if (user.edad == "" || user.edad < 12) { errores.push("Debe colocar una edad válida") }
  if (user.pais == "") { errores.push("Debe seleccionar un país") }
  if (!user.pref1 && !user.pref2) { errores.push("Debe seleccionar al menos una preferencia") }
  if (!user.tipoUsuario) { errores.push("Debe seleccionar un tipo de usuario") }
  if (errores.length > 0) { errores.forEach(error => errUl.innerHTML += `<li>${error}</li>` )
  } else {
